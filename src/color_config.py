...
def get_args():
	...
	parser.add_argument('inputfile', metavar='filename', type=str, nargs=1,
			help='yaml file path to parse')
	parser.add_argument('-d', '--dest', nargs='?',
			help='destination path for generated colorscheme file, default is current directory',
			default=os.getcwd())
	parser.add_argument('-cg', '--callgraph', action='store_true',
			help='generate functions callgraph')
	parser.add_argument('-i', '--indent', action='store_true',
			help='indent colorscheme file using vim editor')
	parser.add_argument('-now', '--no-overwrite', action='store_true',
			help='throw error if target file has already existed')
...
def set_up(colorscheme):
	...
	defined_groups = colorscheme['group'].keys()
...
def has_link():
	""" If link dictionary available in config file """
	return False if color_dict.get('link') is None else True

def get_links(link_index='ALL'):
	""" Return a list of linked groups """
	if link_index == 'ALL':
		return color_dict['link']
	return color_dict['link'][link_index]
...
def write_header():
	...
	with open(output_path, 'w') as file:
		info_list = [get_filename(), get_description(), get_author(), get_last_change(),
				get_license(), get_note()]
		longest_info = max(info_list, key=len)
		bar_length = (len(longest_info) + 13) if (len(longest_info) + 13) <= 100 else 100
		header_bar = '=' * bar_length
...
def write_palette():
	...
	with open(output_path, 'a') as file:
		...
		palette_header = '{:{x}}  {:7}  {:{y}}  {}'.format(*formatter,
				x=max_name_len, y=max_rgb_len)
		bar_length = len(palette_header) if len(palette_header) <= 100 else 100
		palette_bar = '=' * bar_length

		put(file, '" ' + palette_header)
		put(file, '" ' + palette_bar)
...
def write_body():
	...
	with open(output_path, 'a') as file:
		...
		max_group_len = len(max(defined_groups, key=len))
...
def parse_link(link):
	""" return a tuple of from_group and to_group from hi link statement in string """
	if '->' not in link:
		err_msg = "'{}': Missing or invalid assignment notation, which should be '->'"
		raise ValueError(err_msg.format(link))
	parse_result = link.replace(' ', '').split('->')
	if len(parse_result) != 2:
		err_msg = "'{}': Invalid Expression: too many or few operands"
		raise ValueError(err_msg.format(link))
	return parse_result
...
def main():
	...
	colorscheme = parse_yaml(args.inputfile[0])
	output_path = os.path.join(args.dest, colorscheme['name'] + '.vim')
	if os.path.isfile(output_path) and args.no_overwrite:
		raise OSError('{} has already existed'.format(output_path))
