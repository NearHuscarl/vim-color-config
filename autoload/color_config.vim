...
" Last Change: Wed Dec 27 01:49:06 +07 2017
...
function! s:print_variables() " {{{
	call append('$', 'Dependencies:')
	call append('$', 's:has_python3: ' . s:has_python3)
	call append('$', 's:has_pyyaml: ' . s:has_pyyaml)
	call append('$', '')
	call append('$', 'Options:')
	call append('$', 's:plugin_path: ' . shellescape(s:plugin_path))
	call append('$', 's:executable_path: ' . shellescape(s:executable_path))
	call append('$', 'g:color_config_output_path: ' . shellescape(g:color_config_output_path))
	call append('$', 'g:color_config_options: ' . string(g:color_config_options))
	call append('$', 'g:color_config_debug: ' . g:color_config_debug)
	call append('$', '')
endfunction
" }}})
function! s:print_debug_info() " {{{
	call append('$', '')
	call append('$', 'Source: ' . s:cmd)
	call append('$', 'Input: ' . s:input)
	call append('$', 'Dest: ' . s:dest)
	call append('$', 'Parameters: ' . substitute(s:options, '\s$', '', ''))
	call append('$', '')
	call s:print_variables()
endfunction
" }}}
function! s:print_bar(length) " {{{
	call append('$', repeat('=', a:length))
endfunction
" }}}
...
function! s:open_status_win(title) " {{{
	" reuse error window if already exists
...
function! s:run_shell_cmd(command) " {{{
   ...
   let b:win_info = winsaveview()
	...
	if g:color_config_debug
		call s:print_debug_info()
	endif
	call s:print_bar(len(getline(1)) <= 100 ? len(getline(1)) : 100)
